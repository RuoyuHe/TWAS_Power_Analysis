...
source("./power/multivariate/power_functions_multivariate.R")
library(dplyr)
library(tidyr)
library(data.table)
...
for(chr in 1:22)
{
  ...
  for(i in 1:length(real_data_result))
  {
    ...
    s1_sample = real_data_result[i]$FINAL_RESULT$n1*s1_factor
    ...
    powerX1 = 
      power_fn(s1_sample,s2_sample,
               sum(real_data_result[i]$FINAL_RESULT$lm_stage1_X1$residuals^2)/(669), 
               real_data_result[i]$FINAL_RESULT$stage2X1$sigma2, 
               real_data_result[i]$FINAL_RESULT$stage2X1$theta,
               real_data_result[i]$FINAL_RESULT$stage2X1$vartheta)
...
write.table(all_sig,
            paste0("~/TWAS_testing/results/power_curve/gtex/n1/power_table_n1_",s1_factor,"_all.txt"),
            row.names = F)
